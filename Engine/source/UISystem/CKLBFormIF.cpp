...
typedef char AssertNodeUpdateTaskOffset[((KLB_SKIP_SHIPPED_ABI_ASSERTS) || (__SIZEOF_POINTER__ != 8) || (offsetof(NodeUpdateContext, task) == 8)) ? 1 : -1];
typedef char AssertNodeUpdateContextSize[((KLB_SKIP_SHIPPED_ABI_ASSERTS) || (__SIZEOF_POINTER__ != 8) || (sizeof(NodeUpdateContext) == 16)) ? 1 : -1];
