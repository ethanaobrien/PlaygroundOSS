...
namespace TLFX
{
    ...
    class Emitter : public Entity
    {
        ...
        enum InitialAttribute
        {
            ...
            InitialScaleX,
            InitialScaleY,
            InitialSizeXVariation,
            InitialSizeYVariation,
            InitialLifeVariation,
            InitialLife,
            InitialAmount,
            InitialAmountVariation,
            InitialEmissionAngle,
            InitialEmissionRange,
            InitialGlobalVelocity,
            InitialDirection,
            InitialDirectionVariation,
            InitialDirectionVariationOT,
            InitialFramerate,
            InitialStretch,
            InitialSplatter,
        ...
        struct InitialAttributeTable
        {
            InitialAttributeTable() {}

            InitialAttributeTable(const InitialAttributeTable& other)
            {
                for (unsigned int i = 0; i < InitialAttributeCount; ++i)
                    values[i] = other.values[i];
            }

            float& operator[](InitialAttribute attribute) { return values[attribute]; }
            float operator[](InitialAttribute attribute) const { return values[attribute]; }

            float values[InitialAttributeCount];
        };
    ...
    public:
         ...
         * @return Effect*
         */
        Effect* GetParentEffect() const { return _parentEffect; }

        /**
         * Get the tAnimImage currently used by the emitter
         * @return AnimImage*
         */
        AnimImage* GetImage() const { return _image; }

        /**
         * Get the current angle offset used by %angletype (see #setangletype)
         */
        int GetAngleOffset() const { return _angleOffset; }
         ...
         */
        Angle GetAngleType() const { return _angleType; }

        /**
         * Get whether the emitter uses the effect emission instead of its own
         * @return either TRUE or FALSE
         */
        bool IsUseEffectEmmision() const { return _useEffectEmission; }
         ...
         * @return Either TRUE or FALSE
         */
        bool IsSingleParticle() const { return _singleParticle; }

        /**
         * Get whether the emitter chooses a random color for the particles it spawns
         * @return Either TRUE or FALSE
         */
        bool IsRandomColor() const { return _randomColor; }
        ...
        // base
        float GetEmitterLife(unsigned int frame) const { return IsAttributeConstant(0) ? _initialAttributes[InitialLife] : GetCompiledValue(_compiledLife, frame); }
        float GetEmitterLifeVariation(unsigned int frame) const { return IsAttributeConstant(1) ? _initialAttributes[InitialLifeVariation] : GetCompiledValue(_compiledLifeVariation, frame); }
        float GetEmitterAmount(unsigned int frame) const { return IsAttributeConstant(2) ? _initialAttributes[InitialAmount] : GetCompiledValue(_compiledAmount, frame); }
        float GetEmitterSizeX(unsigned int frame) const { return IsAttributeConstant(3) ? _initialAttributes[InitialSizeX] : GetCompiledValue(_compiledSizeX, frame); }
        float GetEmitterSizeY(unsigned int frame) const { return IsAttributeConstant(4) ? _initialAttributes[InitialSizeY] : GetCompiledValue(_compiledSizeY, frame); }
        float GetEmitterBaseSpeed(unsigned int frame) const { return IsAttributeConstant(5) ? _initialAttributes[InitialBaseSpeed] : GetCompiledValue(_compiledBaseSpeed, frame); }
        float GetEmitterBaseWeight(unsigned int frame) const { return IsAttributeConstant(6) ? _initialAttributes[InitialBaseWeight] : GetCompiledValue(_compiledBaseWeight, frame); }
        float GetEmitterBaseSpin(unsigned int frame) const { return IsAttributeConstant(7) ? _initialAttributes[InitialBaseSpin] : GetCompiledValue(_compiledBaseSpin, frame); }
        float GetEmitterEmissionAngle(unsigned int frame) const { return IsAttributeConstant(8) ? _initialAttributes[InitialEmissionAngle] : GetCompiledValue(_compiledEmissionAngle, frame); }
        float GetEmitterEmissionRange(unsigned int frame) const { return IsAttributeConstant(9) ? _initialAttributes[InitialEmissionRange] : GetCompiledValue(_compiledEmissionRange, frame); }
        float GetEmitterSplatter(unsigned int frame) const { return IsAttributeConstant(10) ? _initialAttributes[InitialSplatter] : GetCompiledValue(_compiledSplatter, frame); }
        float GetEmitterVelVariation(unsigned int frame) const { return IsAttributeConstant(18) ? _initialAttributes[InitialVelVariation] : GetCompiledValue(_compiledVelVariation, frame); }
        float GetEmitterWeightVariation(unsigned int frame) const { return IsAttributeConstant(24) ? _initialAttributes[InitialWeightVariation] : GetCompiledValue(_compiledWeightVariation, frame); }
        float GetEmitterAmountVariation(unsigned int frame) const { return IsAttributeConstant(25) ? _initialAttributes[InitialAmountVariation] : GetCompiledValue(_compiledAmountVariation, frame); }
        float GetEmitterSizeXVariation(unsigned int frame) const { return IsAttributeConstant(26) ? _initialAttributes[InitialSizeXVariation] : GetCompiledValue(_compiledSizeXVariation, frame); }
        float GetEmitterSizeYVariation(unsigned int frame) const { return IsAttributeConstant(29) ? _initialAttributes[InitialSizeYVariation] : GetCompiledValue(_compiledSizeYVariation, frame); }
        float GetEmitterSpinVariation(unsigned int frame) const { return IsAttributeConstant(16) ? _initialAttributes[InitialSpinVariation] : GetCompiledValue(_compiledSpinVariation, frame); }
        float GetEmitterDirectionVariation(unsigned int frame) const { return IsAttributeConstant(17) ? _initialAttributes[InitialDirectionVariation] : GetCompiledValue(_compiledDirectionVariation, frame); }
        // over lifetime
        float GetEmitterAlpha(float lifetimePosition) const { return IsAttributeConstant(11) ? _initialAttributes[InitialAlpha] : GetCompiledValueOverTime(_compiledAlpha, lifetimePosition); }
        float GetEmitterR(float lifetimePosition) const { return IsAttributeConstant(19) ? _initialAttributes[InitialR] : GetCompiledValueOverTime(_compiledR, lifetimePosition); }
        float GetEmitterG(float lifetimePosition) const { return IsAttributeConstant(20) ? _initialAttributes[InitialG] : GetCompiledValueOverTime(_compiledG, lifetimePosition); }
        float GetEmitterB(float lifetimePosition) const { return IsAttributeConstant(21) ? _initialAttributes[InitialB] : GetCompiledValueOverTime(_compiledB, lifetimePosition); }
        float GetEmitterScaleX(float lifetimePosition) const { return IsAttributeConstant(22) ? _initialAttributes[InitialScaleX] : GetCompiledValueOverTime(_compiledScaleX, lifetimePosition); }
        float GetEmitterScaleY(float lifetimePosition) const { return IsAttributeConstant(23) ? _initialAttributes[InitialScaleY] : GetCompiledValueOverTime(_compiledScaleY, lifetimePosition); }
        float GetEmitterSpin(float lifetimePosition) const { return IsAttributeConstant(12) ? _initialAttributes[InitialSpin] : GetCompiledValueOverTime(_compiledSpin, lifetimePosition); }
        float GetEmitterVelocity(float lifetimePosition) const { return IsAttributeConstant(13) ? _initialAttributes[InitialVelocity] : GetCompiledValueOverTime(_compiledVelocity, lifetimePosition); }
        float GetEmitterWeight(float lifetimePosition) const { return IsAttributeConstant(14) ? _initialAttributes[InitialWeight] : GetCompiledValueOverTime(_compiledWeight, lifetimePosition); }
        float GetEmitterDirection(float lifetimePosition) const { return IsAttributeConstant(27) ? _initialAttributes[InitialDirection] : GetCompiledValueOverTime(_compiledDirection, lifetimePosition); }
        float GetEmitterDirectionVariationOT(float lifetimePosition) const { return IsAttributeConstant(31) ? _initialAttributes[InitialDirectionVariationOT] : GetCompiledValueOverTime(_compiledDirectionVariationOT, lifetimePosition); }
        float GetEmitterFramerate(float lifetimePosition) const { return IsAttributeConstant(28) ? _initialAttributes[InitialFramerate] : GetCompiledValueOverTime(_compiledFramerate, lifetimePosition); }
        float GetEmitterStretch(float lifetimePosition) const { return IsAttributeConstant(15) ? _initialAttributes[InitialStretch] : GetCompiledValueOverTime(_compiledStretch, lifetimePosition); }
        // global adjusters
        float GetEmitterGlobalVelocity(unsigned int frame)
        {
            if (IsAttributeConstant(30))
                return _initialAttributes[InitialGlobalVelocity];
            unsigned int sample = _compiledGlobalVelocity.GetLastFrame();
            sample = sample >= frame ? frame : sample;
            return _compiledGlobalVelocity.GetValues()[sample * _compiledGlobalVelocity.GetStride()];
        }
    ...
    protected:
        bool IsAttributeConstant(unsigned int bit) const
        {
            return (_constantAttributes & (1U << bit)) != 0;
        }

        static float GetCompiledValue(const CompiledEmitterArray& array, unsigned int frame)
        {
            unsigned int lastFrame = array.GetLastFrame();
            if (frame > lastFrame)
                frame = lastFrame;
            return array.GetValues()[frame * array.GetStride()];
        }

        static float GetCompiledValueOverTime(const CompiledEmitterArray& array, float lifetimePosition)
        {
            int frame = 0;
            if (lifetimePosition > 0)
                frame = (int)(lifetimePosition * array.GetLife());
            unsigned int sample = array.GetLastFrame();
            sample = sample >= (unsigned int)frame ? (unsigned int)frame : sample;
            return array.GetValues()[sample * array.GetStride()];
        }
        ...
        float*                                  _attributeArrays;       /// shared interleaved table for compiled overtime attributes
        ...
        unsigned int                            _constantAttributes;

        // Cached values produced while compiling the editable graphs.
        InitialAttributeTable _initialAttributes;

        // Compact runtime tables, shared by copies of an emitter template.
        CompiledEmitterArray _compiledR, _compiledG, _compiledB;
        CompiledEmitterArray _compiledBaseSpin, _compiledSpin, _compiledSpinVariation;
        CompiledEmitterArray _compiledVelocity, _compiledBaseWeight, _compiledWeight, _compiledWeightVariation;
        CompiledEmitterArray _compiledBaseSpeed, _compiledVelVariation, _compiledAlpha;
        CompiledEmitterArray _compiledSizeX, _compiledSizeY, _compiledScaleX, _compiledScaleY;
        CompiledEmitterArray _compiledSizeXVariation, _compiledSizeYVariation;
        CompiledEmitterArray _compiledLifeVariation, _compiledLife;
        CompiledEmitterArray _compiledAmount, _compiledAmountVariation;
        CompiledEmitterArray _compiledEmissionAngle, _compiledEmissionRange, _compiledGlobalVelocity;
        CompiledEmitterArray _compiledDirection, _compiledDirectionVariation, _compiledDirectionVariationOT;
        CompiledEmitterArray _compiledFramerate, _compiledStretch, _compiledSplatter;
        ...
        // Editable source graphs. Runtime copies deliberately keep these empty.
        EmitterArrayGraph _cR, _cG, _cB;
        EmitterArrayGraph _cBaseSpin, _cSpin, _cSpinVariation;
        EmitterArrayGraph _cVelocity, _cBaseWeight, _cWeight, _cWeightVariation;
        EmitterArrayGraph _cBaseSpeed, _cVelVariation, _cAlpha;
        EmitterArrayGraph _cSizeX, _cSizeY, _cScaleX, _cScaleY;
        EmitterArrayGraph _cSizeXVariation, _cSizeYVariation;
        EmitterArrayGraph _cLifeVariation, _cLife;
        EmitterArrayGraph _cAmount, _cAmountVariation;
        EmitterArrayGraph _cEmissionAngle, _cEmissionRange, _cGlobalVelocity;
        EmitterArrayGraph _cDirection, _cDirectionVariation, _cDirectionVariationOT;
        EmitterArrayGraph _cFramerate, _cStretch, _cSplatter;
    };
