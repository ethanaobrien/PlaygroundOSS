...
namespace TLFX
{
    ...
    bool Entity::Update()
    {
        ...
        {
            const float radians = _direction * ((float)M_PI / 180.0f);
            _pixelsPerSecond = _speed / currentUpdateTime;
            _speedVec.x = sinf(radians) * _pixelsPerSecond;
            _speedVec.y = cosf(radians) * _pixelsPerSecond;
