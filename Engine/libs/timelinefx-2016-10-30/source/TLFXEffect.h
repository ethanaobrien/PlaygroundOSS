...
namespace TLFX
{
    ...
    class Effect : public Entity
    {
        ...
        enum EffectBypass
        {
            ...
            BypassSizeY         = 0x0010,
            BypassHeight        = 0x0040,
            BypassEffectAngle   = 0x0080,
            BypassEmissionAngle = 0x0100,
            BypassEmissionRange = 0x0200,
            BypassGlobalZ       = 0x0400,
            BypassAlpha         = 0x0800,
            BypassSpin          = 0x1000,
            BypassVelocity      = 0x2000,
            BypassWeight        = 0x4000,
            BypassStretch       = 0x8000
        };
        ...
        public:
            ...
            // A value becomes readable only after its corresponding bypass bit
            // is set by compilation, so template construction does not seed it.
            BypassValues() {}

            BypassValues(const BypassValues& other)
            {
                for (int value = 0; value < ValueCount; ++value)
                {
                    m_values[value] = other.m_values[value];
                }
            }

            float& operator[](Value value) { return m_values[value]; }
            float operator[](Value value) const { return m_values[value]; }

        private:
            float m_values[ValueCount];
        };

        static float GetCompiledValue(const CompiledEmitterArray& values, unsigned int frame)
        {
            unsigned int lastFrame = values.GetLastFrame();
            unsigned int index = (frame <= lastFrame) ? frame : lastFrame;
            return values.GetValues()[index * values.GetStride()];
        }
    ...
    public:
         ...
         */
        void SetClass(Type type) { _class = type; }
         ...
         */
        void SetLockAspect(bool value) { _lockAspect = value; }
         ...
         */
        void SetMGX(int value) { _mgx = value; }
         ...
         */
        void SetMGY(int value) { _mgy = value; }
         ...
         */
        void SetEmitAtPoints(bool value) { _emitAtPoints = value; }
         ...
         */
        void SetEmissionType(Emission type) { _emissionType = type; }
         ...
         */
        void SetEffectLength(int length) { _effectLength = length; }
         ...
         */
        void SetParentEmitter(Emitter* emitter) { _parentEmitter = emitter; }

        /**
         * Sets the number of frames
         * Currently only relevant to the Timeline Particles editor
         */
        void SetFrames(int frames) { _frames = frames; }

        /**
         * Sets the anim width
         * Currently only relevant to the Timeline Particles editor
         */
        void SetAnimWidth(int width) { _animWidth = width; }

        /**
         * Sets the anim height
         * Currently only relevant to the Timeline Particles editor
         */
        void SetAnimHeight(int height) { _animHeight = height; }
         ...
         * Currently only relevant to the Timeline Particles editor
         */
        void SetAnimX(int x) { _animX = x; }

        /**
         * Sets the y offset
         * Currently only relevant to the Timeline Particles editor
         */
        void SetAnimY(int y) { _animY = y; }
         ...
         * Currently only relevant to the Timeline Particles editor
         */
        void SetFrameOffset(int offset) { _frameOffset = offset; }
         ...
         */
        void SetTraverseEdge(bool edge) { _traverseEdge = edge; }
         ...
         */
        void SetEndBehavior(End behavior) { _endBehavior = behavior; }
         ...
         * age of the particle.
         */
        void SetDistanceSetByLife(bool value) { _distanceSetByLife = value; }
         ...
         */
        void SetHandleCenter(bool center) { _handleCenter = center; }
         ...
         */
        void SetReverseSpawn(bool reverse) { _reverseSpawn = reverse; }
         ...
         */
        Type GetClass() const { return _class; }

        /**
         * Returns the <i>_lockAspect</i> 
         * @return Either TRUE or FALSE
         */
        bool GetLockAspect() const { return _lockAspect; }
         ...
         */
        int GetAnimWidth() const { return _animWidth; }

        /**
         * Get the height of the animation
         */
        int GetAnimHeight() const { return _animHeight; }
         ...
         */
        int GetAnimX() const { return _animX; }

        /**
         * Get the y offset of the effect in the animation
         */
        int GetAnimY() const { return _animY; }
        ...
        int GetEllipseOffset() const { return _ellipseOffset; }
        ...
        // Lookups
        float GetLife(unsigned int frame) const { return (_bypassers & BypassLife) ? _bypassValues[BypassValues::Life] : GetCompiledValue(_cLife, frame); }
        float GetAmount(unsigned int frame) const { return (_bypassers & BypassAmount) ? _bypassValues[BypassValues::Amount] : GetCompiledValue(_cAmount, frame); }
        float GetSizeX(unsigned int frame) const { return (_bypassers & BypassSizeX) ? _bypassValues[BypassValues::SizeX] : GetCompiledValue(_cSizeX, frame); }
        float GetSizeY(unsigned int frame) const { return (_bypassers & BypassSizeY) ? _bypassValues[BypassValues::SizeY] : GetCompiledValue(_cSizeY, frame); }
        float GetVelocity(unsigned int frame) const { return (_bypassers & BypassVelocity) ? _bypassValues[BypassValues::Velocity] : GetCompiledValue(_cVelocity, frame); }
        float GetWeight(unsigned int frame) const { return (_bypassers & BypassWeight) ? _bypassValues[BypassValues::Weight] : GetCompiledValue(_cWeight, frame); }
        float GetSpin(unsigned int frame) const { return (_bypassers & BypassSpin) ? _bypassValues[BypassValues::Spin] : GetCompiledValue(_cSpin, frame); }
        float GetAlpha(unsigned int frame) const { return (_bypassers & BypassAlpha) ? _bypassValues[BypassValues::Alpha] : GetCompiledValue(_cAlpha, frame); }
        float GetEmissionAngle(unsigned int frame) const { return (_bypassers & BypassEmissionAngle) ? _bypassValues[BypassValues::EmissionAngle] : GetCompiledValue(_cEmissionAngle, frame); }
        float GetEmissionRange(unsigned int frame) const { return (_bypassers & BypassEmissionRange) ? _bypassValues[BypassValues::EmissionRange] : GetCompiledValue(_cEmissionRange, frame); }
        float GetWidth(unsigned int frame) const { return (_bypassers & BypassWidth) ? _bypassValues[BypassValues::Width] : GetCompiledValue(_cWidth, frame); }
        float GetHeight(unsigned int frame) const { return (_bypassers & BypassHeight) ? _bypassValues[BypassValues::Height] : GetCompiledValue(_cHeight, frame); }
        float GetEffectAngle(unsigned int frame) const { return (_bypassers & BypassEffectAngle) ? _bypassValues[BypassValues::EffectAngle] : GetCompiledValue(_cEffectAngle, frame); }
        float GetStretch(unsigned int frame) const { return (_bypassers & BypassStretch) ? _bypassValues[BypassValues::Stretch] : GetCompiledValue(_cStretch, frame); }
        float GetGlobalZ(unsigned int frame) const { return (_bypassers & BypassGlobalZ) ? _bypassValues[BypassValues::GlobalZ] : GetCompiledValue(_cGlobalZ, frame); }

        float GetCurrentAmount() const { return _currentAmount; }
        float GetCurrentLife() const { return _currentLife; }
        float GetCurrentEmissionAngle() const { return _currentEmissionAngle; }
        float GetCurrentEmissionRange() const { return _currentEmissionRange; }
        int GetSpawnDirection() const { return _spawnDirection; }
        float GetCurrentWidth() const { return _currentWidth; }
        float GetCurrentHeight() const { return _currentHeight; }
        float GetCurrentVelocity() const { return _currentVelocity; }
        float GetCurrentSizeX() const { return _currentSizeX; }
        float GetCurrentSizeY() const { return _currentSizeY; }
        float GetCurrentWeight() const { return _currentWeight; }
        float GetCurrentAlpha() const { return _currentAlpha; }
        float GetCurrentSpin() const { return _currentSpin; }
        float GetCurrentStretch() const { return _currentStretch; }

        unsigned int GetLifeLastFrame() const { return _cLife.GetLastFrame(); }
        float GetLifeMaxValue() const { return _lifeGraph.GetMaxValue(); }

        bool IsBypassWeight() const { return _bypassWeight; }

        void SetParticlesCreated(bool value) { _particlesCreated = value; }
        ...
        void SetCurrentEffectFrame(unsigned int frame) { _currentEffectFrame = frame; }
        unsigned int GetCurrentEffectFrame() const { return _currentEffectFrame; }
    ...
    protected:
        unsigned int                   _bypassers;              /// Bit mask of attributes whose compiled values can be bypassed
        ...
        unsigned int                   _currentEffectFrame;     /// current lookup-table frame for the effect
        ...
        // Constant values cached while compiling each graph.  When a graph
        // has only one sample the update path can use this value directly.
        BypassValues                   _bypassValues;

        // Compact runtime lookup tables.
        CompiledEmitterArray           _cLife;
        CompiledEmitterArray           _cAmount;
        CompiledEmitterArray           _cSizeX;
        CompiledEmitterArray           _cSizeY;
        CompiledEmitterArray           _cVelocity;
        CompiledEmitterArray           _cWeight;
        CompiledEmitterArray           _cSpin;
        CompiledEmitterArray           _cAlpha;
        CompiledEmitterArray           _cEmissionAngle;
        CompiledEmitterArray           _cEmissionRange;
        CompiledEmitterArray           _cWidth;
        CompiledEmitterArray           _cHeight;
        CompiledEmitterArray           _cEffectAngle;
        CompiledEmitterArray           _cStretch;
        CompiledEmitterArray           _cGlobalZ;

        // Editable source graphs retained by library-owned effect templates.
        EmitterArrayGraph              _lifeGraph;
        EmitterArrayGraph              _amountGraph;
        EmitterArrayGraph              _sizeXGraph;
        EmitterArrayGraph              _sizeYGraph;
        EmitterArrayGraph              _velocityGraph;
        EmitterArrayGraph              _weightGraph;
        EmitterArrayGraph              _spinGraph;
        EmitterArrayGraph              _alphaGraph;
        EmitterArrayGraph              _emissionAngleGraph;
        EmitterArrayGraph              _emissionRangeGraph;
        EmitterArrayGraph              _widthGraph;
        EmitterArrayGraph              _heightGraph;
        EmitterArrayGraph              _effectAngleGraph;
        EmitterArrayGraph              _stretchGraph;
        EmitterArrayGraph              _globalZGraph;
        bool                           _arrayOwner;             // instances borrow their template's compiled lookup storage
