...
namespace TLFX
{
    ...
    {
        return Open(_doc.load_file(filename));
    }

    bool PugiXMLLoader::Open( const char *buffer, unsigned int size )
    {
        return Open(_doc.load_buffer(buffer, size));
    }

    bool PugiXMLLoader::Open( pugi::xml_parse_result result )
    {
        _error[0] = 0;

        if (!result)
        {
            sprintf(_error, "Parsing error at #%d : %s", result.offset, result.description());
            return false;
        }
        if (!_doc.child("EFFECTS"))
        {
            strcpy(_error, "Root element <EFFECTS> is missing");
            return false;
        }
    ...
    bool PugiXMLLoader::GetNextShape( AnimImage *shape )
    {
        ...
        if (!_currentShape)
        {
            strcpy(_error, "No more shapes there");
            return false;
        }
    ...
    Effect* PugiXMLLoader::GetNextEffect(const std::list<AnimImage*>& sprites)
    {
        if (!_currentEffect)
        {
            strcpy(_error, "No more effects there");
            return NULL;
        }
    ...
    Effect* PugiXMLLoader::GetNextSuperEffect(const std::list<AnimImage*>& sprites)
    {
        if (!_currentEffect)
        {
            strcpy(_error, "No more super effects there");
            return NULL;
        }
