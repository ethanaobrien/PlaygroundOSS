...
float EffectsLibrary::_lookupFrequencyOverTimeInverse = 1.0f;
...
bool EffectsLibrary::Load( const char *filename, bool compile /*= true*/ )
{
    ...
    bool loaded = loader->Open(filename);
    if (loaded)
        Load(loader, compile);
...
bool EffectsLibrary::Load(const char *buffer, unsigned int size, bool compile)
{
    XMLLoader *loader = CreateLoader();
    bool loaded = loader->Open(buffer, size);
    if (loaded)
        Load(loader, compile);
...
void EffectsLibrary::Load(XMLLoader *loader, bool compile)
{
    AnimImage *shape;
...
void EffectsLibrary::SetLookupFrequencyOverTime( float freq )
{
    ...
    _lookupFrequencyOverTimeInverse = freq > 0.0f ? 1.0f / freq : 0.0f;
}
